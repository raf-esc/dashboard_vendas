...
with st.sidebar.expander('Valor do frete'):
    frete = st.slider('Selecione o valor',
                      0,
                      300,
                      (0, 300))
...
with st.sidebar.expander('Local da compra'):
    locais = st.multiselect('Selecione os locais',
                            dados['Local da compra'].unique(),
                            dados['Local da compra'].unique())
with st.sidebar.expander('Avaliação da compra'):
    avaliacao = st.slider('Selecione o valor',
                          0,
                          5,
                          (0, 5))
with st.sidebar.expander('Tipo de pagamento'):
    tipos_pagamento = st.multiselect('Selecione os tipos',
                                    dados['Tipo de pagamento'].unique(),
                                    dados['Tipo de pagamento'].unique())
with st.sidebar.expander('Quantidade de parcelas'):
    parcelas = st.slider('Selecione a quantidade',
                         0,
                         24,
                         (0, 24))
